"""E2e edit-then-validate tests for real scene fixtures (Godot 4.4.1).
...
Note: 4.4.1 gamepiece.tscn has 6 nodes (Node2D > Decoupler > Path2D >
PathFollow2D > CameraAnchor + GFXAnchor), richer than other versions.
"""
...
BAD_SCENES_DIR = Path(__file__).resolve().parents[1] / "data" / "scenes" / "bad_4_4_1"
...
# Scenes with ≥3 nodes (suitable for move tests)
...
class TestDeleteNodeGodotCheck:
    ...
    def test_delete_leaf(self, scene_name):
        # 4.4.1 gamepiece has deeper hierarchy: Decoupler/Path2D/PathFollow2D/CameraAnchor
        leaf = {
            "gamepiece.tscn": "Decoupler/Path2D/PathFollow2D/CameraAnchor",
    ...
    def test_delete_subtree_gamepiece_decoupler(self):
        """Delete Decoupler — removes 5-node subtree (4.4.1 specific)."""
        cli_exit, godot_exit = _edit_and_validate(
            "gamepiece.tscn", ["delete-node", "{scene}", "Decoupler"]
        )
        assert cli_exit == 0, "CLI validate failed: gamepiece Decoupler delete"
        assert godot_exit == 0, "Godot check failed: gamepiece Decoupler delete"
...
class TestComplexChangesGodotCheck:
    ...
    def test_pathfollow_progress_gamepiece(self):
        # 4.4.1: PathFollow2D is at Decoupler/Path2D/PathFollow2D
        cli_exit, godot_exit = _edit_and_validate(
            "gamepiece.tscn",
            [
                "update-properties",
                "{scene}",
                "Decoupler/Path2D/PathFollow2D",
                "--property",
                "progress=50.0",
                "--property",
                "h_offset=10.0",
...
class TestTreeCommand:
    ...
    def test_tree_json_gamepiece_4_4_1(self):
        """4.4.1 gamepiece has 6 nodes with Decoupler hierarchy."""
        _require_scene("gamepiece.tscn")
        r = run_cli_edit(["tree", str(SCENES_DIR / "gamepiece.tscn"), "--json"])
        assert r.exit_code == 0
        payload = json.loads(r.stdout)
        assert payload["name"] == "Gamepiece"
        assert payload["type"] == "Node2D"
        children = payload.get("children", [])
        assert len(children) == 1
        assert children[0]["name"] == "Decoupler"
