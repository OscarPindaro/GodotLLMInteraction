"""E2e test: parse real .tscn files from the open-rpg 4.4.1 project.

These scenes are from gdquest-demos/godot-open-rpg at commit 47972ef
("Upgrade to Godot 4.4.1"), MIT licensed. They are real-world scenes with
scripts, sub-resources, signals, and various node types.
...
def test_gamepiece_scene_has_node_hierarchy():
    """gamepiece.tscn should have a Decoupler > Path2D > PathFollow2D hierarchy."""
    scene = load_scene(_scene_path("gamepiece.tscn"))
    assert scene.nodes[0].type == "Node2D"
    assert scene.nodes[1].type == "Node"
    assert scene.nodes[1].name == "Decoupler"
    assert scene.nodes[2].type == "Path2D"
    assert scene.nodes[3].type == "PathFollow2D"
    assert scene.nodes[4].type == "RemoteTransform2D"
    assert scene.nodes[5].type == "RemoteTransform2D"
