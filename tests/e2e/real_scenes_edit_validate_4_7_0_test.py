"""E2e edit-then-validate tests for real scene fixtures (Godot 4.7).
...
VERSION = "4.7"
SCENES_DIR = Path(__file__).resolve().parents[1] / "data" / "scenes" / "open_rpg_4_7_0"
BAD_SCENES_DIR = Path(__file__).resolve().parents[1] / "data" / "scenes" / "bad_4_7_0"
...
# Scenes with >1 node (suitable for delete/rename/move tests)
MULTI_NODE_SCENES = [
    "gamepiece.tscn",
    "ScreenTransition.tscn",
    "Trigger.tscn",
...
class TestExportedPropertiesGodotCheck:
    ...
    def test_update_existing_exported_float(self):
        """Update max_speed (already overridden in scene) to a new value."""
        cli_exit, godot_exit = _edit_and_validate(
            "exported_props.tscn",
    ...
    def test_update_existing_exported_int(self):
        """Update damage (already overridden in scene) to a new value."""
        cli_exit, godot_exit = _edit_and_validate(
            "exported_props.tscn",
    ...
    def test_update_existing_exported_bool(self):
        """Update is_active (already overridden in scene) back to true."""
        cli_exit, godot_exit = _edit_and_validate(
            "exported_props.tscn",
    ...
    def test_add_exported_string_not_in_scene(self):
        """Set label (matches default, not in scene) to a new value."""
        cli_exit, godot_exit = _edit_and_validate(
            "exported_props.tscn",
    ...
    def test_add_exported_color_not_in_scene(self):
        """Set tint (matches default, not in scene) to a new value."""
        cli_exit, godot_exit = _edit_and_validate(
            "exported_props.tscn",
            [
                "update-properties",
    ...
    def test_add_exported_vector2_not_in_scene(self):
        """Set spawn_position (matches default, not in scene) to a new value."""
        cli_exit, godot_exit = _edit_and_validate(
            "exported_props.tscn",
            [
                "update-properties",
    ...
    def test_multi_exported_property_update(self):
        """Update multiple exported properties of different types at once."""
        cli_exit, godot_exit = _edit_and_validate(
            "exported_props.tscn",
            [
                "update-properties",
    ...
    def test_exported_properties_roundtrip(self):
        """Chained: update all 6 exported properties, then validate."""
        binary = _require_binary()
...
class TestTreeCommand:
    ...
    def test_tree_json_trigger(self):
        ...
        r = run_cli_edit(
            ["tree", str(SCENES_DIR / "Trigger.tscn"), "--json"],
        )
...
class TestBadScenesCliGodotAgree:
    ...
    def test_both_reject_bad_scene(self, scene_name):
        ...
        # CLI side: tree command (parse) — should fail for parse errors
        tree_result = run_cli_edit(["tree", str(scene_path), "--json"])

        # CLI side: validate command — delegates to Godot --check-only
        ...
        # CLI and Godot must agree on the scene's validity.
