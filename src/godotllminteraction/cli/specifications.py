...
# --- Per-version global enum generation -------------------------------------
#
# Mirrors the `global_enums` section of extension_api.json. Each enum becomes
# a Python IntEnum with the original Godot member names (KEY_A, MOUSE_BUTTON_LEFT,
# etc.). Dotted enum names (Variant.Type, Variant.Operator) are flattened to
# valid Python class names (VariantType, VariantOperator). A BY_NAME dict maps
# the original Godot enum name to the generated class for runtime lookup.


def _flatten_enum_class_name(name: str) -> str:
    """Flatten dotted Godot enum names to valid Python class names.

    'Variant.Type' -> 'VariantType', 'Key' -> 'Key'.
    """
    return name.replace(".", "")


def _extract_global_enums(
    data: dict,
) -> list[tuple[str, bool, list[tuple[str, int]]]]:
    """(enum_name, is_bitfield, [(member_name, value), ...]) sorted by enum name."""
    result: list[tuple[str, bool, list[tuple[str, int]]]] = []
    for enum in data.get("global_enums", []):
        name = enum["name"]
        is_bitfield = enum.get("is_bitfield", False)
        values = sorted(
            ((v["name"], v["value"]) for v in enum.get("values", [])),
            key=lambda t: t[0],
        )
        result.append((name, is_bitfield, values))
    return sorted(result, key=lambda t: t[0])


def render_global_enums_source(data: dict, version: str) -> str:
    """Render one IntEnum class per global enum in extension_api.json.

    Member names keep Godot's full spelling (KEY_A, MOUSE_BUTTON_LEFT, ...).
    Dotted enum names (Variant.Type) are flattened to valid Python identifiers
    (VariantType). A ``BY_NAME`` dict maps the original Godot enum name to the
    generated class for runtime lookup.
    """
    enums = _extract_global_enums(data)

    parts = [
        '"""Auto-generated by `gli specifications generate-global-enums '
        f'--version {version}`. Do not edit by hand."""',
        "",
        "from __future__ import annotations",
        "",
    ...
    for enum_name, _is_bitfield, values in enums:
        class_name = _flatten_enum_class_name(enum_name)
        parts.append("")
        ...
        if not values:
            ...
            for member_name, member_value in values:
                parts.append(f"    {member_name} = {member_value}")
    parts.append("")
    parts.append("")
    parts.append("BY_NAME = {")
    for enum_name, _is_bitfield, _values in enums:
        class_name = _flatten_enum_class_name(enum_name)
        parts.append(f"    {_quote(enum_name)}: {class_name},")
    parts.append("}")
    parts.append("")
    return "\n".join(parts)


@app.command("generate-global-enums")
def generate_global_enums(
    ...
    """Generate global_enums.py for a specifications/<version>/ package.

    One IntEnum class per entry in the `global_enums` section of
    extension_api.json (Key, MouseButton, JoyButton, ...). Member names keep
    Godot's full spelling; a BY_NAME dict maps original enum names to the
    generated classes for runtime lookup.
    """
    if not _VERSION_RE.match(version):
        print_error(
            f"Invalid version {version!r}; expected a format like 'v4_7_0' "
    ...
    data = json.loads(api_json.read_text())
    source = render_global_enums_source(data, version)

    root = specs_root or _SPECIFICATIONS_ROOT
    version_dir = root / version
    enums_path = version_dir / "global_enums.py"

    if check:
        if not version_dir.exists() or _is_stale(enums_path, source):
            print_error(
                f"specifications/{version}/global_enums.py is missing or stale. "
                "Run `gli specifications generate-global-enums` to refresh."
            )
            raise typer.Exit(code=EXIT_ERROR)
        print_success("Generated global_enums.py is up to date.")
        return
    ...
    if _write_if_changed(enums_path, source):
        print_success(f"Updated {enums_path}.")
    else:
        print_text("Generated global_enums.py already up to date.")
...
def add_version(
    ...
    generate_global_enums(version=version, api_json=api_json, specs_root=specs_root)
